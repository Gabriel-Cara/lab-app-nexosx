# Redesign de UI — Histórico, Convenções e Lições Aprendidas
...
> **Status: o redesign planejado (Grupos A, B e C) está concluído** (ver seção "Grupo C avaliado" abaixo). Não há lote pendente. Este documento continua sendo a referência viva de **convenções de UI, componentes reutilizáveis e pegadinhas já resolvidas** — consulte antes de mexer em qualquer tela visual do `app/web`/`app/lab`. Renomeado de `redesign-proximas-etapas.md` (2026-08-27) porque o nome original não refletia mais o conteúdo.

Companion visual: `redesign-ui-historico.canvas` (abre no Obsidian, mesmo vault).
...
- ~~`docs/ui/ui-redesign/`~~ — v1 (primeira exploração, cores rejeitadas — "ficou HORRIVEL", roxo/indigo genérico de IA); **removido na reorganização de docs de 2026-08-27** já que o v2 tem o mesmo conjunto de páginas e é a versão aprovada — este parágrafo é o único registro que sobrou dele
