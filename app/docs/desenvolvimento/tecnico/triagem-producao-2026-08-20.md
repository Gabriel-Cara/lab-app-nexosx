...
> **Status: bloqueantes e achados menores todos resolvidos** (rate-limit por IP mantido deliberadamente como está — ver seção "Achados menores"). **Um item de "Próximos passos" segue genuinamente pendente e não rastreado em nenhum outro lugar: conformidade LGPD, nunca avaliada** — ver `checklist-implementacao.md`, backlog da Fase 13.

Escopo: com o roadmap de produto em 98% (ver `checklist-implementacao.md`) e a auditoria de segurança de 15/08 fechada (`auditoria-seguranca-2026-08-15.md`), o usuário pediu uma triagem completa do sistema — back e front, dentro e fora do navegador, com **carga e concorrência reais** (nunca testadas antes; todo teste anterior foi manual, um usuário/uma ação por vez) — pra decidir se já vale prospectar venda/aluguel do produto ou se é melhor melhorar antes.
...
| --- | --- | --- | --- |
| Formulário "Novo chamado" não marca "Descrição" como obrigatório visualmente, mas bloqueia o envio com toast genérico sem destacar o campo | `pages/app/maintenance-requests.tsx` | Baixa — UX confusa, não bloqueia o uso | ✅ CORRIGIDO (2026-08-25) — `aria-required`/`aria-invalid` nos campos Título/Descrição, reaproveitando a regra CSS global já existente (`[data-slot="field"]:has([aria-required])` → asterisco vermelho no label; `aria-invalid` → borda vermelha) |
| Porteiro tentando ação restrita a gestor (ex: desativar morador) não é mais deslogado (já corrigido), mas o botão continua visível e o erro é um toast genérico sem indicar que é falta de permissão | `components/residents/deactivate-button.tsx` e pontos similares | Baixa — o backend já bloqueia corretamente, é só polimento de UX | ✅ CORRIGIDO (2026-08-25) — botão de ativar/desativar e "Importar planilha" (também manager/admin-only) agora só renderizam pro papel que pode de fato executar a ação, em vez de aparecer e falhar |
| Painel admin (Dashboard/Solicitações/Condomínios/Usuários) nunca passou pelo trabalho de redesign visual feito nas outras telas nesta sessão — funciona, mas visualmente cru comparado ao resto do app | `pages/admin/*.tsx` | Baixa — cosmético, não afeta função | ✅ CORRIGIDO (2026-08-25) — as 4 telas ganharam `PageHeader`, `Card`/`CardHeader`/`CardContent` (tabelas que estavam soltas sem `Card`), e Solicitações ganhou `SegmentedFilter` no lugar do `Select` solto |
| Rate-limit de login é por IP, não por conta — correto e não contornável, mas num condomínio com Wi-Fi/NAT compartilhado, vários logins legítimos simultâneos da mesma rede podem levar 429 junto, sem ser ataque nenhum | `api/src/middlewares/rate-limit.ts` | Observacional — monitorar depois do lançamento, não corrigir preventivamente | ⏸️ Mantido como está, por decisão explícita do usuário (2026-08-25) — só mexer se virar reclamação real |
| Falta teste dedicado pra `condominiums-controller.ts` (única rota de provisionamento direto condomínio+gestor pelo admin, fora do fluxo de solicitação já auditado) | `api/src/controllers/condominiums-controller.ts` | Baixa — item de higiene de teste, não risco de segurança ativo | ✅ CORRIGIDO (2026-08-25) — `condominiums-controller.test.ts` novo, 3 testes (duplicata de código rejeitada, provisionamento em transação com role forçado server-side, listagem) |

**Todos os itens não bloqueantes desta triagem foram resolvidos em 2026-08-25**, exceto o rate-limit por IP, que foi mantido deliberadamente.
